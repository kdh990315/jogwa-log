...
import { ArrowUpRightIcon } from "../icons/arrow-up-right/arrow-up-right";
import { ChevronLeftIcon } from "../icons/chevron-left/chevron-left";
import { ChevronRightIcon } from "../icons/chevron-right/chevron-right";
    ...
    <Card className="flex h-full flex-col justify-between border-none p-4 shadow-sm hover:shadow-md">
      <div className="mb-1.5 flex items-start justify-between gap-2">
        ...
        <div className="ml-2 shrink-0 rounded-lg bg-[color:var(--brand-surface)] p-1.5">
          <MetricIcon className="h-4 w-4 text-[color:var(--brand-fg)]" name={metric.icon} />
        </div>
      </div>
        ...
        <div className="mt-1 flex flex-wrap items-center gap-1 text-[11px] font-medium">
          {metric.change ? (
            hasPositiveChange ? (
              ...
              <span className="text-[color:var(--fg-muted)]">{metric.change}</span>
            )
          ) : null}
  ...
  return (
    <Card className="flex h-full flex-col border-none p-4 shadow-sm lg:col-span-2">
      <div className="mb-4 flex items-center justify-between">
        <h3 className="text-base font-bold text-[color:var(--fg)]">출조 캘린더</h3>
      ...
      <div className="hidden overflow-x-auto md:block">
        <table className="w-full text-left text-xs">
          <thead className="border-b border-[color:var(--line)] bg-[color:var(--surface-muted)] font-medium text-[color:var(--fg-faint)]">
            <tr>
              <th className="rounded-tl-lg px-3 py-2.5 whitespace-nowrap">포인트 명</th>
              <th className="px-3 py-2.5 whitespace-nowrap">주력 어종</th>
              <th className="px-3 py-2.5 text-center whitespace-nowrap">출조 횟수</th>
              <th className="px-3 py-2.5 text-center whitespace-nowrap">평균 조과</th>
              <th className="px-3 py-2.5 text-center whitespace-nowrap">평균 사이즈</th>
              <th className="rounded-tr-lg px-3 py-2.5 text-right whitespace-nowrap">성공률</th>
            </tr>
          </thead>
          <tbody>
            {locations.map((location) => (
              ...
              >
                ...
                <td className="px-3 py-3 font-bold text-[color:var(--fg)] whitespace-nowrap">
                  {log.count > 0 ? (
                    ...
                    <span className="font-medium text-[color:var(--fg-muted)]">꽝</span>
                  )}
                </td>
                {isSea ? (
